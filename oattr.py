...
"""
   This module parses OPeNDAP Hyrax DMR++ file and print the attributes.
"""
...
class DMRParser(object):
    ...
    def get_unsigned_attr(self, attr_node, dtype, value):
        ...
        if ("isUnsigned" in attr_node.keys() and 
            attr_node.attrib["isUnsigned"] == 'true'):
            if (value[0] == '[' and value[-1] == ']'):
                cvalue = ''
                value_list = ast.literal_eval(value)
                for item in value_list:
                    cdtype, tvalue = self.get_unsigned_dtype_value(dtype, 
                                                                   str(item))
    ...
    def get_unsigned_dtype_dset(self, dset_node, dtype):
        ...
        for node in dset_node.getchildren():
            if node.tag == self.schema + "attribute":
                if (node.attrib["name"] == '_Unsigned' and 
                    node.attrib["value"] == 'true'):
                    cdtype = 'u'+dtype
        return cdtype
    ...
    def get_value(self, node):
        ...
        for c in node.getchildren():
            if (c.tag == self.schema + 'value' or
                c.tag == self.schema + 'Value'):
                v.append(c.text)
        if v == []:
            print('WARNING:No value/Value tag is found for ' + \
                  node.attrib["name"] + '.')
        return v
    ...
    def get_orig_gname(self, g):
        ...
        l = ['Data_Fields', 'Geolocation_Fields', 'HDFEOS_INFORMATION', 
...
class TestDMRParser(unittest.TestCase):
    ...
    def setUp(self):
        """set file name."""
        f = 'OMI-Aura_L2-OMNO2_2016m0215t0210-o61626_v003-2016m0215t200753.he5.h5.dmrpp'
        self.parser = DMRParser(f)
